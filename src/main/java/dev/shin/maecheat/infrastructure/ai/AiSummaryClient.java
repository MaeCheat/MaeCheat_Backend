...
//        }
        ...
        String prompt = """
                ...
                특히 닉네임이 메이플스토리 직업 계열(모험가, 시그너스 기사단, 영웅, 레지스탕스, 노바, 초월자, 프렌즈 월드, 레프, 아니마),
                직업 분류(전사, 궁수, 마법사, 법사, 도적, 해적),
                미전직 직업(초보자, 노블레스, 시티즌, 데몬),
                또는 직업명(히어로, 팔라딘, 다크나이트, 아크메이지, 비숍, 보우마스터, 신궁, 패스파인더, 나이트로드, 섀도어, 듀얼블레이드, 바이퍼, 캡틴, 캐논슈터, 소울마스터, 플레임위자드, 윈드브레이커, 나이트워커, 스트라이커, 미하일, 아란, 에반, 메르세데스, 팬텀, 은월, 루미너스, 데몬슬레이어, 데몬어벤져, 블래스터, 배틀메이지, 와일드헌터, 메카닉, 제논, 카이저, 카인, 카데나, 엔젤릭버스터, 제로, 키네시스, 아델, 일리움, 칼리, 아크, 렌, 라라, 호영)과
                일치하거나 포함하는 경우, 게시글이 해당 직업/직업군을 일반적으로 언급하는 것인지 특정 유저를 지칭하는 것인지 **매우 신중하게** 판단해주세요.
                일반적으로, **님(**에는 직업명 혹은 직업군명)으로 지칭하는 경우에는 해당 캐릭터만을 지칭하는 의미가 아닐 가능성이 매우 높습니다.
                또한, '그 도적' 혹은 '그 전사' 같은 직업명 혹은 직업군명을 사용한 모호한 지칭으로 표현된 경우에도 연관이 없는 것으로 판단해주세요.
